...
export default function Portfolio() {
  const { data: portfolioItems, isLoading: portfolioLoading } = trpc.portfolio.list.useQuery({});
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            ...
            ) : portfolioItems && portfolioItems.filter(item => item.category === "mural").length > 0 ? (
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
                {portfolioItems
                  .filter(item => item.category === "mural")
                  .map((item) => (
                    <div
                      key={item.id}
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            ...
            ) : portfolioItems && portfolioItems.filter(item => item.category === "3d-model" || item.category === "diorama").length > 0 ? (
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
                {portfolioItems
                  .filter(item => item.category === "3d-model" || item.category === "diorama")
                  .map((item) => (
                    <div
                      key={item.id}
                      className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition"
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            ...
            ) : portfolioItems && portfolioItems.filter(item => item.category === "workshop").length > 0 ? (
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
                {portfolioItems
